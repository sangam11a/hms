        ...
        $('document').ready(function(){
            ...
            function handle_pagination(){
                ...
                document.getElementById("reports").innerHTML="";
                   var x="<table class='table table-dark'><thead><th>S.N</th><th>Inventory Name</th><th>Quantity</th><th> Selling Price</th><th>Cost price</th><th>User</th><th>Date</th><th>Time</th><th>Profit</th></thead>"; 
                var start=$("#starting_date").val();
                var end=$("#ending_date").val();
                    var sql="Select * from utilized_inventory where date1>='"+start+"' and date1<='"+end+"' order by id desc";
                        ...
                        success:function(data){
                            ...
                            if(data.length>=paginateValue){
                                ...
                                for(i=0;i<paginateValue;i++){
                                    x+="<tbody><td>"+Number(i+1)+"</td><td>"+data[i]['name']+"</td><td>"+data[i]['quantity']+"</td><td>"+data[i]['selling_price']+"</td><td>"+data[i]['cost_price']+"</td><td>"+data[i]['user']+"</td><td>"+data[i]['date1']+"</td><td>"+data[i]['time1']+"</td><td>"+data[i]['profit']+"</td></tbody>";
                                }
                                x+="</table>";
